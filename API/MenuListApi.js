  ...
  dsDon = async () => {
    ...
    if (token != '') {
      var response = await fetch(
        'http://222.252.26.108:8889/api/app-shipper/shipment/findShipmentByShipperId?size=10&page=0',
        {
          method: 'GET',
          headers: {
            ACCESS_TOKEN:
              'Bearer ' +
              'eyJhbGciOiJIUzI1NiJ9..WkOfvfLDVIyspwzdc3krZR3HUIsR96labcdjuMmXVsI',
          },
        },
      )
    ...
    return (
      <TouchableOpacity
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <Text style={{fontSize: 15}}> {item.shippingFromName}</Text>
        </View>
        <View style={{flexDirection: 'row'}}>
          <Text style={{fontSize: 15}}>Địa Chỉ Shop :</Text>
          <Text style={{fontSize: 13}}> {item.shippingFromAddress}</Text>
        </View>
        <View style={{flexDirection: 'row'}}>
          <Text style={{fontSize: 15}}>SĐT Shop :</Text>
          <Text style={{fontSize: 15}}> {item.shippingFromPhone}</Text>
        </View>
      </TouchableOpacity>
    );
  };
